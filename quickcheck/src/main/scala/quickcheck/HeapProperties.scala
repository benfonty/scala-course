...
trait HeapProperties(val heapInterface: HeapInterface):
  ...
  val meldingHeaps: (String, Prop) =
    def check(meltedHeap: List[Node], heap1: List[Node], heap2: List[Node]): Boolean =
      if isEmpty(meltedHeap) then
        true
      else if !isEmpty(heap1) && findMin(meltedHeap) == findMin(heap1) then
        check(deleteMin(meltedHeap), deleteMin(heap1), heap2)
      else if !isEmpty(heap2) && findMin(meltedHeap) == findMin(heap2) then
        check(deleteMin(meltedHeap), heap1, deleteMin(heap2))
      else
        false
    ...
    forAll { (heap1: List[Node], heap2: List[Node]) =>
      check(meld(heap1, heap2), heap1, heap2)
    }
